...
class AssignTripDetailsScreen extends StatefulWidget {
  final int tripId;
  final bool fromConflictNavigation;

  const AssignTripDetailsScreen({
    Key? key,
  ...
  @override
  _AssignTripDetailsScreenState createState() => _AssignTripDetailsScreenState();
}

class _AssignTripDetailsScreenState extends State<AssignTripDetailsScreen> {
  ...
  void _updateActionButtons() {
    ...
    // PENDING trips: no buttons
    if (status == 'pending') {
      _showStartButton = false;
      _showEndButton = false;
    }
    // APPROVED trips: show Start button (no meter reading check)
    else if (status == 'approved') {
      _showStartButton = false;
      _showEndButton = false;
    }
    // READ status: show Start button
    else if (status == 'read') {
      _showStartButton = true;
      _showEndButton = false;
    }
    // ONGOING trips: show End button
  ...
  Widget _buildActionButtons() {
    //if (_isLoading || _tripDetails == null) return SizedBox.shrink();
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (_showEndButton)
            ElevatedButton(
              onPressed: _isActionLoading ? null : _endTrip,
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.red,
            ...
            if (_tripDetails?.status == "approved")
            Container(
              padding: EdgeInsets.all(16),
              decoration: BoxDecoration(
                color: Colors.red,
  ...
  void _navigateToConflictTrip(int tripId) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            AssignTripDetailsScreen(tripId: tripId, fromConflictNavigation: true),
      ),
    );
  }
  ...
  Widget _buildContent() {
    return Column(
      children: [
        ...
        if(_showStartButton || _showEndButton || _tripDetails?.status == "approved")_buildActionButtons(),
      ],
    );
  }
