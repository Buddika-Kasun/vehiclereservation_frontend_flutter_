...
class ApprovalTripsScreen extends StatefulWidget {
  final VoidCallback? onBackToApprovalConfig;

  const ApprovalTripsScreen({Key? key, this.onBackToApprovalConfig}) : super(key: key);

  @override
  _ApprovalTripsScreenState createState() => _ApprovalTripsScreenState();
}

class _ApprovalTripsScreenState extends State<ApprovalTripsScreen> {
  ...
  Future<void> _searchUsers(String query) async {
    ...
    _searchTimer = Timer(const Duration(milliseconds: 500), () async {
      try {
        final response = await ApiService.searchTripsApprovalUsers(query);
        if (response['success'] == true) {
          final usersData = response['data']['users'] as List<dynamic>;
          setState(() {
  ...
  try {
    ...
    final response = await ApiService.tripApproveUser(userId, true);
  ...
  try {
    ...
    final response = await ApiService.tripApproveUser(userId, false);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Text(
            'Emergency trips approval users',
            style: TextStyle(
              fontSize: 16,
              fontWeight: FontWeight.bold,
