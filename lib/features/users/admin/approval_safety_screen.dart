...
class ApprovalSafetyScreen extends StatefulWidget {
  final VoidCallback? onBackToApprovalConfig;

  const ApprovalSafetyScreen({Key? key, this.onBackToApprovalConfig}) : super(key: key);

  @override
  _ApprovalSafetyScreenState createState() => _ApprovalSafetyScreenState();
}

class _ApprovalSafetyScreenState extends State<ApprovalSafetyScreen> {
  ...
  try {
    ...
    final response = await ApiService.getUsersBySafetyApproval();
  ...
  Future<void> _searchUsers(String query) async {
    ...
    _searchTimer = Timer(const Duration(milliseconds: 500), () async {
      try {
        final response = await ApiService.searchSafetyApprovalUsers(query);
        if (response['success'] == true) {
          final usersData = response['data']['users'] as List<dynamic>;
          setState(() {
  ...
  try {
    ...
    final response = await ApiService.safetyApproveUser(userId, true);
  ...
  try {
    ...
    final response = await ApiService.safetyApproveUser(userId, false);
