...
abstract class BaseAllVehiclesListState<T extends StatefulWidget>
    extends State<T> {
  // Common properties
  List<Vehicle> allVehicles = [];
  List<Vehicle> displayedAllVehicles = [];
  ...
  // Abstract methods
  Future<void> fetchAllVehicles({required bool reset, bool silent = false});
  ...
  Future<void> _silentRefresh() async {
    ...
    try {
      ...
      if (currentPage == 1) {
        ...
        _cachedAllVehicles = List.from(allVehicles);
        await fetchAllVehicles(reset: true, silent: true);
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  void _scrollListener() {
    ...
    if (scrollController.position.pixels >=
      ...
      if (hasMoreData && !loadingMore && !isLoading) {
        _loadMoreAllVehicles();
      }
    }
  }

  Future<void> _loadMoreAllVehicles() async {
    ...
    await fetchAllVehicles(reset: false);
  ...
  bool hasDataChanged(
    ...
    List<Vehicle>? oldList,
  ]) {
    final compareList = oldList ?? _cachedAllVehicles;
    ...
    for (int i = 0; i < newList.length; i++) {
      ...
      if (newItem.id != oldItem.id ||
          newItem.assignedDriverPrimaryId != oldItem.assignedDriverPrimaryId ||
  ...
  void updateAllVehiclesSilently(
    List<Vehicle> newList,
    int newTotal,
    bool newHasMore,
  ) {
    ...
    if (_cachedAllVehicles.isEmpty && newList.isEmpty) {
      if (kDebugMode) {
    ...
    if (hasDataChanged(newList, _cachedAllVehicles)) {
      if (kDebugMode) {
        print('🔄 Silent refresh: Data changed, updating UI');
        print(
          '   Old count: ${_cachedAllVehicles.length}, New count: ${newList.length}',
        );
      }
      setState(() {
        allVehicles = newList;
        displayedAllVehicles = newList;
        total = newTotal;
