...
class _CreateTripScreenState extends State<CreateTripScreen> {
  ...
  void _autoCalculateRoute() async {
    ...
    final stopsWithCoordinates = stops
        .where(
          (stop) => stop.coordinates != null && stop.controller.text.isNotEmpty,
        )
        .length;
  ...
  Future<void> _setLocationFromCoordinates(
    LatLng position,
    RouteStop stop,
  ) async {
    try {
      ...
      stop.controller.text =
          "${position.latitude.toStringAsFixed(6)}, ${position.longitude.toStringAsFixed(6)}";
      stop.coordinates =
          position; // Set coordinates even if reverse geocoding fails
      _addOrUpdateMarkerForStop(stop, position);
  ...
  void _addOrUpdateMarkerForStop(RouteStop stop, LatLng pos) {
    ...
    final markerKey = ValueKey(
      '${stop.type}_${stop.controller.hashCode}_$stopIndex',
    );

    // Remove existing marker for this stop
    markers.removeWhere((m) {
      final existingKey = m.key as ValueKey?;
      return existingKey?.value.toString().contains(stop.type.toString()) ==
              true &&
          existingKey?.value.toString().contains(
                stop.controller.hashCode.toString(),
              ) ==
              true;
    });
  ...
  Future<void> _calculateRoute() async {
    ...
    try {
      ...
      for (var i = 0; i < stops.length; i++) {
        ...
        try {
          final query =
              "${stop.coordinates?.latitude.toStringAsFixed(6)}, ${stop.coordinates?.longitude.toStringAsFixed(6)}";
  ...
  */
  void _proceedToNext() {
    ...
    if (stops[0].controller.text.isEmpty ||
        stops.last.controller.text.isEmpty) {
      _showMessage('Please enter start and destination locations');
      return;
    }
  ...
  Widget _buildMap() {
    return FlutterMap(
      ...
      children: [
        ...
        if (routeSegments.isNotEmpty) PolylineLayer(polylines: routeSegments),
            ...
            Expanded(
              child: Scrollbar(
                ...
                child: ListView(
                  controller: _scrollController,
                  children: [
                    ...visibleStops.asMap().entries.map(
                      (e) => _buildStopInput(e.key, e.value),
                    ),
...
class RouteStop {
  ...
  RouteStop({required this.controller, required this.type, this.coordinates});
}
